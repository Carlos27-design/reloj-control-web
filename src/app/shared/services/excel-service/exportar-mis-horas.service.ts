...
export class ExportarMisHorasService {
  constructor() {}

  exportarExcel(registros: Registro[]) {
    const libro = new Exceljs.Workbook();
    const hoja = libro.addWorksheet('RegistrosHoraTrabajador');

    hoja.columns = [
      { header: 'fecha', key: 'fecha', width: 10 },
      { header: 'Entradas', key: 'Entradas', width: 10 },
      { header: 'Salidas', key: 'Salidas', width: 10 },
      { header: 'Trabajador', key: 'Trabajador', width: 10 },
    ];

    registros.forEach((registro) => {
      const fecha = new Date(registro.entrada);
      ...
      hoja.addRow({
        ...
        Trabajador:
          registro.Trabajador.nombre + ' ' + registro.Trabajador.apellido,
      });
    });
